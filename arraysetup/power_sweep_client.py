...
atten30 = Attenuator("mix", gain=-30)
device_atten = Device("mkid", gain=-3)
atten20 = Attenuator("4K", gain=-20)
atten10 = Attenuator("cable atten", gain=-10)

signal_chain = ROChain([dac_atten, atten20, atten10, atten30, device_atten, paramp, hemt, if1, adc_atten1, if2, adc_atten2, if3, if4, if5, atten1, if6])

dac_atten_start, adc_attens_start = calculate_adc_dac_atten(-33, -95, signal_chain, -1)
adc_atten_start = sum(adc_attens_start) #TODO: there is a rare (potentially impossible?) edge case to be handeled here

dac_atten_stop, _ = calculate_adc_dac_atten(-33, -110, signal_chain, -1)

compute_power_sweep_attenuations(dac_atten_start, adc_atten_start, dac_atten_stop)
