...
def adc_test_plot(adc_data, timerange, fft_range, fft_zoom,  fs=4.096e9, figsize=(13, 8), **mosaic_kw):
...
def plot_fft(f, y, log=False, xlim=(-2.048e9, 2.048e9), ylim=None, ax=None):
    if ax is not None:
        plt.sca(ax)
    plt.plot(f, y)
    plt.grid(True)
