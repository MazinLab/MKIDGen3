...
class CaptureSink(threading.Thread):
    ...
    def kill(self):
        if self._pipe:
            getLogger('zmqflow.captures').debug(f'Killing {self} and closing command pipe...')
            self._pipe[0].send(b'')
            getLogger('zmqflow.captures').debug(f'sent..')
            self._pipe[0].close()
            getLogger('zmqflow.captures').debug(f'done')
    ...
    def run(self):
        ...
        try:
            ...
            self.destablish()
            # getLogger('zmqflow.captures').debug(f'Closing sink thread internal pipe for {self}')
            # self._pipe[1].close()
