...
class MCS2MMBufferChain:
    def __init__(self, n, buffer_size=8096, contiguous=True, zero_buffers=True):
        ...
        for i, buf_addr in zip(range(n), self.bd_addr):
            descriptor = MCS2MMBufferDescriptor(self._chain.device_address + 0xC0 * (i + 1), buf_addr, buffer_size)
            self._chain[i * 16:(i + 1) * 16] = descriptor.array

    @property
    def head(self):
        return self.bd_addr[0]

    @property
    def tail(self):
        return self.bd_addr[-1]
...
class MCDMA(DefaultIP):
    ...
    ADDR_S2MM_CHCR = {i: 0x540 + i * 0x40 for i in range(16)}
    ...
    def config_recieve(self, n_buffers=2, buffer_size_bytes=8096, channels=tuple(range(16))):
        """
        ...
        for c in channels:
            self.write(MCDMA.ADDR_S2MM_CURDESC[c], self.bd_chains[c].head.addr & 0xffff)
            self.write(MCDMA.ADDR_S2MM_CURDESC[c], self.bd_chains[c].head.addr >> 32)

        # 3. Program the CHANNEL.RS bit of channel control registers. After this point the CDs cannot be reprogrammed.
        for c in range(16):
            x = self.read(MCDMA.ADDR_S2MM_CHCR[c])
            x = x | 1 if x in channels else x ^ 1  # clear run bits on other channels
            self.write(MCDMA.ADDR_S2MM_CHCR[c], x)
        ...
        for c in channels:
            ...
            self.write(MCDMA.ADDR_S2MM_CHCR[c], reg)
        ...
        for c in channels:
            self.write(MCDMA.ADDR_S2MM_TAILDESC[c], self.bd_chains[c].tail.addr & 0xffff)
            self.write(MCDMA.ADDR_S2MM_TAILDESC[c], self.bd_chains[c].tail.addr >> 32)
...
class DACTableAXIM(DefaultIP):
    ...
    def replay(self, data, tlast=True, tlast_every=256, length=None, start=True, fpgen=lambda x: FP16_15(x).__index()):
        ...
        self._buffer[1:data.size * 2:2] = [fpgen(x) for x in data.imag] if fpgen is not None else data.real
...
class IQCapture(DefaultIP):
    ...
    def capture(self, n, groups=tuple(range(N_IQ_GROUPS)), start=True):
        ...
        self.write(self.ADDR_CAPTURESIZE, n*n_groups)
        for i, k in enumerate(keep):
            self.write(self.ADDR_KEEP+0x4*i, k)
        if start:
            self.start_capture()
