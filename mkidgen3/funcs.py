...
class Waveform:
    ...
    def compute_waveform(self):
        ...
        for i in range(self.freqs.size):
            ...
            c1 = self.iq_ratios[i] * scaled * np.exp(1j * self.phase_offsets_rad[i])
            iq.real += c1.real * exp.real + c1.imag * exp.imag
            iq.imag += scaled * exp.imag
        self.values = iq
        return iq

    def optimize_random_phase(self, max_quant_err=None, max_attempts=10):
        """
        inputs:
        ...
        self.quant_vals, self.quant_error = quantize_to_int(self.values, resolution=DAC_RESOLUTION, signed=True, word_length=ADC_DAC_INTERFACE_WORD_LENGTH,
                                return_error=True)
        cnt=0
        while self.quant_error>max_quant_err:
            ...
            self.quant_vals, self.quant_error = quantize_to_int(self.values, resolution=DAC_RESOLUTION, signed=True,
